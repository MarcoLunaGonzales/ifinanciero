...
<table id="tablePaginator" class="table table-condensed table-bordered">
  ...
  <?php
    ...
    <tr style="background-color:#E3CEF6;">
      ...
      <th class="text-left small"colspan="3"><small>Glosa</small></th>           
	...
	<tr>
		...
		<td class="text-left small" colspan="3"><?=$glosaComprobante;?></td>
  ...
  <tbody>        
      <?php
      	$sqlDetalle="SELECT * from comprobantes_detalle where cod_comprobante=$codigo_cobt order by orden";
		...
		while ($row = $stmtDetalle->fetch(PDO::FETCH_BOUND)) {
			...
			<tr>
				...
				<button rel="tooltip" class="<?=$buttonEdit;?>" onclick="SeleccionarComprobante_cajachica_reembolso('<?=$codigo_cobt?>','<?=$codigo_detalle?>','<?=$glosa_detalle;?>','<?=$monto?>')">SELECCIONAR          
				</button>
				</td>
			</tr>
		<?php }
