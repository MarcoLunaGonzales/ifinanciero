            ...
            <tbody id="tabla_archivos">
              ...
              if(isset($cod_facturacion)){
                ...
                while ($rowArchivo = $stmtArchivo->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  $onClick='onClick="quitarArchivoSistemaAdjunto_solfac('.$filaA.','.$codigoArchivoX.',1,1)"';
                     $downloadFile='download="Doc - IFINANCIERO ('.$nombreX.')"';
                     if(obtenerValorConfiguracion(93)==1){
                      $banderaArchivo=obtenerBanderaArchivoIbnorca('archivos_adjuntos_solicitud_facturacion',$codigoArchivoX);
                      if($banderaArchivo>0){
                         ...
                         $onClick='onClick="ajaxDeleteArchivoIbnorcaFacturacion(\''.$globalServerDelete.'\',\''.$banderaArchivo.'\',\'divArchivo\',16,\''.$codigoArchivoX.'\','.$filaA.','.$codigoArchivoX.',1,1);"';
                      }                      
                     }
                  ?>
