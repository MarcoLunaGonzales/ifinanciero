...
$porcentaje_pagar = $_POST["porcentaje_pagar"];
try{
    ...
    if($cont_tipopago!=0){//falta asociar cuenta a tipos de pago 
        ...
        if($cod_sucursal==null || $cod_sucursal==''){//sucursal no encontrada
            ...
            if($nroAutorizacion==null || $nroAutorizacion=='' || $nroAutorizacion==' '){
                ...
                $totalFinalRedondeado=round($monto_pagar,0);
                ...
                if($flagSuccess){
                  ...
                  $stmtNroFac = $dbh->prepare("SELECT codigo from facturas_venta where cod_solicitudfacturacion=$codigo order by codigo desc LIMIT 1");
                    ...
                    { 
                        ...
                        $precio_x=$porcentaje_pagar*$precio_x/100;//formula para obtener el monto con el porcentaje a pagar
