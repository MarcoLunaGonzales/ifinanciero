...
if($interno_delete==0){
	$url_retorno=$urllistFacturasServicios;
}else{
	$url_retorno=$urllistFacturasServicios2	;
}
...
$stmtFActuras = $dbh->prepare("SELECT codigo,fecha_factura,nro_factura,nit,razon_social,cod_comprobante,cod_unidadorganizacional,cod_area,idTransaccion_siat from facturas_venta where codigo in ($codigos_facturas_x)");
...
$stmtFActuras->bindColumn('idTransaccion_siat', $idTransaccion_siat);
$cadenaFacturas="";
...
// $datosAnulacion=anulacionFacturaSiat($idTransaccion_siat);//anulacion desde SIAT
//PROMISES
// print_r($datosAnulacion);
// if(isset($datosAnulacion->estado)){
	// $estado_anulacion_siat=$datosAnulacion->estado;
	// if($estado_anulacion_siat==1){
		...
		//     Swal.fire({
		//     title: 'IBNO SIAT: TRANSACCION EXITOSA :)',	    
		//     type: 'error'
		//     }).then(function() {
	...
	// 	echo "<script language='Javascript'>
	// 	    Swal.fire({
	// 	    title: 'IBNO SIAT: ".$datosAnulacion->mensaje." :)',	    
	// 	    type: 'error'
	// 	    }).then(function() {
	// 	        location.href='../".$url_retorno."';
	// 	    });
...
// 	echo "<script language='Javascript'>
// 		    Swal.fire({
// 		    title: 'IBNO SIAT: ERROR AL ANULAR LA FACTURA :(',	    
// 		    type: 'error'
// 		    }).then(function() {
// 		        location.href='../".$url_retorno."';
// 		    });
