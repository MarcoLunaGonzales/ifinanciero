...
<html><head>
  ...
  <div class="pagina">
    ...
    <div class="pt-2 s-10 pl-6">
        <div class="">A/A: &nbsp;&nbsp;<?=obtenerValorOferta($codOferta,8,$default,2)?></div>
        <div class="pl-6 font-weight-bold"><?=obtenerValorOferta($codOferta,8,$default,3)?></div>
...
$poderIbnorca=" El INSTITUTO BOLIVIANO DE NORMALIZACIÓN Y CALIDAD (IBNORCA), asociación sin fines de lucro legalmente constituida, con NIT Nº 1020745020, que en virtud al Testimonio de Poder Nº 427/2020 de fecha 14 de agosto de 2020 otorgado por ante Notaría de Fe Pública de Primera Clase Nº 097 del Distrito Judicial de La Paz, a cargo de la Dra. Patricia Ampuero Carrillo se encuentra debidamente representado en el presente acto por el Sr. José Jorge Durán Guillén mayor de edad, hábil por derecho, con C.I. Nº 461774 LP y que en lo sucesivo a los fines del presente contrato se denominará simplemente “IBNORCA”.";
    ...
    <div class="s-9 ">
        ...
        <div class="pl-6 pr-6 text-justificar">
            <p class="font-weight-bold">La certificación de producto con SELLO IBNORCA y la emisión de documento de conformidad de producto según Reglamento Técnico RM 261.2018, aplica para los productos:</p>
            <p class="pl-2">
            <?php 

             $stmtAtributos = $dbh->prepare("SELECT * from simulaciones_servicios_atributos where cod_simulacionservicio=$codigo and nombre!=''");
             ...
             while ($rowAtributo = $stmtAtributos->fetch(PDO::FETCH_ASSOC)) {
               ...
               $normaXAtrib=str_replace(",","",$rowAtributo['norma']);
               $nombreAtrib=str_replace("•","",$nombreAtrib); 
               ?>
               <p class="">•  <?=$nombreAtrib?></p> Marca Comercial: <b><?=$marcaAtrib?></b><br> Segun la Norma: <b><?=$normaXAtrib?></b><br>
               <?php
             }
            ?>
            </p>
            <p>Fabricados en la planta de producción: <?php 
            $datosAtributo=obtenerPrimerAtributoSimulacionServicioDatos($codigo);
            $datosAtributo[0]=str_replace(",","",$datosAtributo[0]);
             echo $datosAtributo[2].",".$datosAtributo[0]." - ".$datosAtributo[1]?></p>
            <p>Además de las normas indicadas, los documentos de referencia para la realización de la auditoría serán:</p>
            <p><b style=""><?=obtenerValorOferta($codOferta,8,$default,5);?></b></p>
        </div>
    </div>
    <div class="s-9 ">
    ...
    <div class="s-9 ">
        ...
        <?php 
         for ($i=1; $i <=$anioX ; $i++) { 
             ...
             switch ($i) {
               case 1:$tituloAnio="Para la auditoría de <b>certificación/renovación</b>";break;
               case 2:$tituloAnio="Para la auditoría de <b>seguimiento I</b>";break;
               case 3:$tituloAnio="Para la auditoría de <b>seguimiento II</b>";break;
               default:$tituloAnio=$ordinal." año (GESTIÓN ".$gestionInicio.")";break;
             }
             ...
             $cantidadPr="SELECT count(*) as cantidad FROM simulaciones_servicios_tiposervicio s, cla_servicios t where s.cod_simulacionservicio=$codigo and s.cod_claservicio=t.IdClaServicio and s.habilitado=1 and s.cod_anio=$i order by t.nro_orden";
        ...
        <table class="table table-bordered">
                <tr class="s-10 text-white bg-danger text-center font-weight-bold">
                    ...
                    <td width="10%">DÍAS <br>AUDITORES</td>
                    <td width="20%">COSTO USD</td>
                </tr>
                <!--<tr class="bg-plomo-claro">
                   <td colspan="4"><?=$tituloAnio?></td>  
                </tr>-->
                <?php 
    ...
    <div class="s-9">
        ...
        <div class="s-9 ">
          ...
          <div class="text-justificar">
            ...
            <img src="../assets/libraries/img/logos_oferta/oferta_new.jpg" alt="NONE" width="100%" height="375px">
          </div>
     ...
     <br>
        <div class="text-justificar">
          <br>
            ...
            <p>Concluida la auditoría, IBNORCA emitirá la correspondiente factura, debiendo el CLIENTE realizar el pago correspondiente a más tardar dentro de los siguientes <?=$diasFormaPago?> días de recibida la misma.</p><p> La factura correspondiente por Derecho de uso de Sello IBNORCA, se emitirá cada gestión en el mes aniversario del certificado emitido, debiendo el CLIENTE realizar el pago correspondiente a más tardar dentro de los siguientes <?=$diasFormaPago?> días de recibida la misma.<p></p> En caso que el CLIENTE no pague el monto de la factura en el plazo señalado, el CLIENTE pagará a IBNORCA, el 2 % de interés sobre el monto adeudado.</p>     
