...
$stmt = $dbh->prepare("SELECT tcc.nombre,cc.numero,cc.observaciones,ccd.codigo,ccd.nro_recibo,cc.cod_comprobante,fdcc.nro_factura
From tipos_caja_chica tcc, caja_chica cc,caja_chicadetalle ccd,facturas_detalle_cajachica fdcc where tcc.codigo=cc.cod_tipocajachica and cc.codigo=ccd.cod_cajachica and ccd.codigo=fdcc.cod_cajachicadetalle and cc.cod_comprobante>0 and cc.cod_estadoreferencial=1 and tcc.codigo=$tipo_cajachica  GROUP BY ccd.codigo ORDER BY ccd.nro_recibo");
    $stmt->execute();    
