...
$html.='<br><table class="table">'.
           '<tbody>'; 
           $index=1;
           $tBolActivo=0;$tBolPasivo=0;$tBolPasivoFull=0;
// Preparamos
...
while ($row = $stmt->fetch(PDO::FETCH_BOUND)) { 
      ...
      while ($row = $stmt2->fetch(PDO::FETCH_BOUND)) {
         ...
         while ($row = $stmt3->fetch(PDO::FETCH_BOUND)) {            
            ...
            while ($row = $stmt4->fetch(PDO::FETCH_BOUND)) {
              ...
              $detallesReporte=listaSumaMontosDebeHaberComprobantesDetalle($fechaFormateada,1,$unidades,$areas,$codigo_4,$gestion,"none");
               ...
               while ($rowComp = $detallesReporte->fetch(PDO::FETCH_ASSOC)) {
                   ...
                   if($codigo==1){
                    $montoX=(float)($rowComp['total_debe']-$rowComp['total_haber']);
                    $tBolActivo+=number_format($montoX, 2, '.', '');  
                  }else{
                    //$montoX=abs((float)($rowComp['total_debe']-$rowComp['total_haber']));
                    //LE CAMBIAMOS EL SIGNO AL PASIVO Y PATRIMONIO
                    $montoX=$montoX*(-1);
                    $tBolPasivo+=number_format($montoX, 2, '.', '');
                    if($codigo==3){
                      $vacio++;
                    }
                  }
                    $sumaNivel4+=number_format($montoX, 2, '.', '');  
               ...
               if($codigo_4==obtieneCuentaPadre($cuentaResultado)&&$vacio==0){                  
                  ...
                  $sumaNivel4+=number_format($montoResultado, 2, '.', '');
              ...
              if(number_format($sumaNivel4, 2, '.', '')>0){
                $sumaNivel3+=number_format($sumaNivel4, 2, '.', '');  
            ...
            if(number_format($sumaNivel3, 2, '.', '')>0){
              $sumaNivel2+=number_format($sumaNivel3, 2, '.', '');
          ...
          if(number_format($sumaNivel2, 2, '.', '')>0){
            $sumaNivel1+=number_format($sumaNivel2, 2, '.', '');
    ...
    $monto=0;
    if($numero[0]!="1"){
      $tBolPasivoFull+=number_format($sumaNivel1, 2, '.', '');
    }
 ...
 $html.=    '</tbody></table>';
      ...
      $tBolPasivo=$tBolPasivo+number_format($montoResultado, 2, '.', '');
     ...
     $html.='<tr class="">'.
                  ...
                  '<td class="text-right">'.number_format($tBolPasivoFull, 2, '.', ',').'</td>'.     
              '</tr>';
  $html.=    '</tbody></table>';
