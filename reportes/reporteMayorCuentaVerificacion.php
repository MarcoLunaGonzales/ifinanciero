  ...
  <div class="table-responsive">
    ...
    $html='<table class="table table-bordered table-condensed" id="libro_mayor_rep">'.
            ...
            '<tr class="text-center">'.
              ...
              '<th width="5%">CodEC</th>'.
              '<th width="5%">CodCuentaEC</th>'.
              '<th width="5%">codCuentaAuxiliarEC</th>'.
              '<th width="5%">MontoEC</th>'.
            '</tr>'.
           '</thead>'.
           '<tbody>';
    ...
    for ($xx=0; $xx < cantidadF($codcuenta); $xx++) { 
      ...
      if($porciones[1]=="aux"){
        ...
        $query1="SELECT d.codigo as cod_det,d.cod_area,d.cod_unidadorganizacional,d.glosa,d.debe,d.haber, p.codigo as cod_cuenta,p.nro_cuenta,p.nombre,d.cod_cuentaauxiliar, u.abreviatura,a.abreviatura as areaAbrev, c.cod_unidadorganizacional as unidad,c.fecha, (c.codigo) as codigo_comprobante
        ...
        $query1="SELECT d.codigo as cod_det,d.cod_area,d.cod_unidadorganizacional,d.glosa,d.debe,d.haber,
          p.codigo as cod_cuenta,p.numero,p.nombre,d.cod_cuentaauxiliar,
      ...
      $sqlEC="SELECT e.codigo, e.cod_plancuenta, e.monto, e.cod_cuentaaux from estados_cuenta e where e.cod_comprobantedetalle='$codigoX'";
      $stmtEC = $dbh->prepare($sqlEC);
      $stmtEC->execute();
      $codigoEC=0;
      $codCuentaEC=0;
      $montoEC=0;
      $codCuentaAuxEC=0;
      while ($rowEC = $stmtEC->fetch(PDO::FETCH_ASSOC)) {
        $codigoEC=$rowEC['codigo'];
        $codCuentaEC=$rowEC['cod_plancuenta'];
        $montoEC=$rowEC['monto'];
        $codCuentaAuxEC=$rowEC['cod_cuentaaux'];
      }

      $txtMontoDif="";
      $diferenciaRep=abs($montoEC-$montoComprobanteDetalle);
      if(abs($montoEC-$montoComprobanteDetalle)>0.5){
        $txtMontoDif="text-danger font-weight-bold";
      }

      $txtCuentaAux="";
      if($codCuentaAuxiliar!=$codCuentaAuxEC){
        $txtCuentaAux="text-danger font-weight-bold";
      }

      $txtCuenta="";
      if($codCuentaX!=$codCuentaEC){
      ...
      $html.='<tr style="background-color: '.$txtColorFila.'">'.
                ...
                '<td class="text-right font-weight-bold small">'.$codigoEC.'</td>'.
                '<td class="text-right '.$txtCuenta.'">'.$codCuentaEC.'</td>'.
                '<td class="text-right '.$txtCuentaAux.'">'.$codCuentaAuxEC.'</td>'.
                '<td class="text-right '.$txtMontoDif.'">'.formatNumberDec($montoEC).'</td>';                
              $html.='</tr>';
