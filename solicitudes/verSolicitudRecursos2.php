            ...
            <div class="col-sm-12">
              ...
              <table class="table table-warning table-bordered table-condensed">
                ...
                <tbody id="tabla_archivos">
                  <?php
                  /*SACAMOS LOS CONTRATOS SI EXISTEN*/
                  $sqlContratos="SELECT IF (s.codigo IS NULL,c.Etapa,s.cod_anio) AS cod_anio,IF (s.codigo IS NULL,ibnorca.d_clasificador (c.idTipoAuditor),s.descripcion) AS descripcion,t.nombre AS tipo,c.IdContrato,ibnorca.id_estadoobjeto (c.IdTipoObjeto,c.IdContrato) AS idEstado,ibnorca.d_clasificador (ibnorca.id_estadoobjeto (c.IdTipoObjeto,c.IdContrato)) AS estado,se.Codigo,ibnorca.d_cliente (se.IdCliente) AS cliente,ibnorca.d_cliente (c.idPersona) AS persona,c.Dias,c.Monto,c.Inicio,c.Fin,ibnorca.d_clasificador (se.IdOficina) AS oficina,c.idPersona,IF (c.CodAuditor IS NULL,c.Etapa,s.cod_anio) AS Etapa,c.Nro,c.Tema,c.TipoMoneda,c.DetPagos,c.TipoPago,c.Factura,IF (c.CodAuditor IS NULL,c.idTipoAuditor,s.cod_tipoauditor) AS idTipoAuditor,ss.descripcion_servicio,se.idPropuesta,(
                      SELECT pathArchivo FROM dbdocumentos.DocumentosFirmados WHERE idObjeto=c.IdContrato AND idTipoObjeto=c.idTipoObjeto AND IFNULL(Estado,0)<> 3 ORDER BY idDocumentoFirmado DESC LIMIT 1) AS ruta FROM ibnorca.contratos c LEFT JOIN bdifinanciero.simulaciones_servicios_auditores s ON s.codigo=c.CodAuditor LEFT JOIN bdifinanciero.tipos_auditor t ON s.cod_tipoauditor=t.codigo INNER JOIN ibnorca.servicios se ON se.IdServicio=c.IdObjeto LEFT JOIN bdifinanciero.simulaciones_servicios ss ON ss.codigo=se.idPropuesta WHERE ibnorca.id_estadoobjeto (c.IdTipoObjeto,c.IdContrato)=3810 AND se.IdServicio='$idServicioX' AND c.idPersona in (SELECT sd.cod_proveedor FROM solicitud_recursosdetalle sd WHERE sd.cod_solicitudrecurso=$codigo)";
                  //echo $sqlContratos;
                  $stmtContratos = $dbh->prepare($sqlContratos);
                  $stmtContratos->execute();
                  while ($rowContratos = $stmtContratos->fetch(PDO::FETCH_ASSOC)) {
                    $URLContrato=$rowContratos['ruta'];
                    $URLContratoCompleto="C:/".$URLContrato;
                    $etapaContrato=$rowContratos['Etapa'];
                    $URLContrato2="../mostrarArchivos.php?name=".$URLContratoCompleto;
                  ?>  
                    <tr id="fila_archivo0" style="background-color: lightgreen;">
                    <td class="text-left">Contrato Registrado en Sistema</td>
                    <td class="text-center">-</td>
                    <td class="text-right">
                      <small id="existe_archivo_cabecera0"></small>
                        <small id="label_txt_documentos_cabecera0"></small> 
                      ...
                      <div class="btn-group">
                        ...
                        <!--a class='btn btn-sm btn-default' href='<?=$URLContratoCompleto;?>' <?=$downloadFile?>><i class='material-icons'>vertical_align_bottom</i></a-->
                        <a class='btn btn-sm btn-primary' href='#' onclick='vistaPreviaArchivoSol("<?=$URLContrato2?>","Descargar: Doc - IFINANCIERO (<?=$nombreX?>)"); return false;'><i class='material-icons'>remove_red_eye</i></a>
                      
                      </div>     
                    </td>    
                    <td>Contrato Etapa:<?=$etapaContrato;?></td>
                  </tr>
                  <?php
                  ...
                  while ($rowArchivo = $stmtArchivo->fetch(PDO::FETCH_ASSOC)) {
                     ...
                     if(obtenerValorConfiguracion(93)==1){
                      ...
                      if($banderaArchivo>0){
                        ...
                        //$urlArchivoMostrar="../mostrarArchivos.php?name="."C:/ibn_archivos/contratos/3800/2021-351/c-F2-817-32.pdf";
