...
$parametros=array("sIdentificador"=>$sIde, "sKey"=>$sKey, "lista"=>"HijoPadre", "padre"=>"55");
...
foreach ($detalle as $objDet){
	...
	//echo $codigo;
	$stmtBusquedaTipoGd = $dbh->prepare("SELECT codigo from tipos_genero where codigo=:codigo");
    ...
    if($codigoG==$codigoG2){//UPDATE
    	$stmtGU = $dbh->prepare("UPDATE tipos_genero set codigo=:codigo,nombre=:nombre, abreviatura=:abreviatura
        where codigo = :codigo");
        //bind
    	...
    	$stmtGI = $dbh->prepare("INSERT INTO tipos_genero(codigo,nombre,abreviatura,cod_estadoreferencial) values(:codigo,:nombre,:abreviatura,:cod_estadoreferencial)");
        //bind
