...
if(isset($_GET['cod_factura'])){
	$codigo_factura=$_GET['cod_factura'];
	GenerarComprobanteExistente($codigo_factura);
}else{
...
function GenerarComprobanteExistente($codigo_factura){
	$fechaActual="2022-08-18";//fecha para estados de cuenta
	...
	try{
		...
		if($codComprobante==0 || $codComprobante=='' || $codComprobante==null){
	        ...
	        if($cod_solicitudfacturacion!=-100){ //diferente a la tienda
				...
				while ($row_detTipopago = $stmtDetalleTipoPago_x->fetch()) {
					...
					if($cod_tipopago==$cod_tipopago_deposito_cuenta){
						...
						// $cod_estado_cuenta_x=-100;//no hay de ningun lugar realizarlo manualmente
						// $cod_compte_origen=obtenerCod_comprobanteDetalleorigen($cod_estado_cuenta_x);
						$cod_compte_origen=-100;
			...
			}//se borró el else. eran  casos desde la tienda
