            ...
            if($flagSuccess){
               ...
               while ($rowPre = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                for ($i=1;$i<=$cantidad_filas;$i++){                
                    ...
                    $sql="INSERT INTO solicitudes_facturaciondetalle(cod_solicitudfacturacion,cod_claservicio,cantidad,precio,descripcion_alterna,descuento_por,descuento_bob,tipo_item,fecha_inicio,fecha_fin) 
                    values ('$cod_facturacion','$servicioInsert_ajax','$CantidadInsert_ajax','$importeInsert_ajax','$DescricpionInsert_ajax','$descuento_por_Insert_ajax','$descuento_bob_Insert_ajax',2,'$fecha_inicio','$fecha_fin')";
            ...
            if($flagSuccess){
                ...
                for ($i=1;$i<=$modal_numeroservicio-1;$i++){
                    ...
                    if($servicioInsert!=0 || $servicioInsert!=""){
                        $stmt = $dbh->prepare("INSERT INTO solicitudes_facturaciondetalle(cod_solicitudfacturacion,cod_claservicio,cantidad,precio,descripcion_alterna,descuento_por,descuento_bob,tipo_item,fecha_inicio,fecha_fin) 
                        values ('$cod_facturacion','$servicioInsert','$CantidadInsert','$importeInsert','$DescricpionInsert','$descuento_por_Insert','$descuento_bob_Insert',2,'$fecha_inicio','$fecha_fin')");//porque fue introducido desde servicios adicionales
                        $flagSuccess=$stmt->execute();
                    }
                }
                 //los agregados con  ajax
