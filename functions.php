...
function obtenerNombreInstanciaCajaChica($codCaja){
 ...
 function precioNormasPropuesta($idPropuesta){
   $dbh = new Conexion();
   $sql="select (sum(v.precio_digital)*0.15)as montonorma from ibnorca.vw_catalogonormas v, simulaciones_costosnormas s where s.cod_norma=v.IdNorma and s.cod_simulacion=$idPropuesta;";
   $stmt = $dbh->prepare($sql);
   $stmt->execute();
   $valor=0;
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
/**
 * Función que consume el servicio para actualizar los datos de un cliente
 *
 * @param int $idCliente El ID del cliente que se actualizará.
 * @param string $razonSocial La nueva razón social del cliente.
 * @param string $nit El nuevo NIT del cliente.
 * @param int $idTipoDocumento El ID del nuevo tipo de documento asociado al cliente.
 * @param int $idSolicitud El ID de la solicitud de la facturacion.
 *
...
function wsModificaCliente($idCliente,$razonSocial,$nit,$idTipoDocumento,$idSolicitud){
    ...
    curl_exec($ch);
    curl_close($ch);
    return 'Finalizo el servicio Cliente de Contacto';
}
