...
<div class="card-body">
  ...
  <div class="table-responsive">
    ...
    $listaDetalle=obtenerListaGastosPorArea($unidadCostoArray,$areaCostoArray,$desde,$hasta);
    $totalImporte=0;
    while ($rowComp = $listaDetalle->fetch(PDO::FETCH_ASSOC)) {
        $codAreaX=$rowComp['cod_area'];
        $areaX=$rowComp['area'];
        $importe_realX=$rowComp['monto_real'];
        ...
        for($i=0; $i<$longitudUnidades; $i++){
          ...
          $listaDetalleUnidades=obtenerListaGastosPorArea($unidadCosto[$i],$codAreaX,$desde,$hasta);
          while ($rowCompUnidades = $listaDetalleUnidades->fetch(PDO::FETCH_ASSOC)) {
            $importe_realY=$rowCompUnidades['monto_real'];
            if($importe_realY>0){
              $html.='<tr">'.
