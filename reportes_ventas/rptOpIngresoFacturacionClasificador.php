            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header <?=$colorCard;?> card-header-icon">
                  ...
                  <h4 class="card-title">Reporte Ingresos por Clasificador</h4>
                </div>
                <form class="" action="<?=$urlReporteIngresoFacturacion_clasificador?>" target="_blank" method="POST">
                  ...
                  <div class="row">
			                		...
			                		<div id="div_contenedor_oficina_costo">
											...
											?>
												<select class="selectpicker form-control form-control-sm" name="unidad_costo[]" id="unidad_costo" data-style="select-with-transition" multiple data-actions-box="true" required data-live-search="true">
													...
													?>
												      	 <option value="<?=$row["codigo"];?>" selected data-subtext="<?=$row["nombre"];?>" <?=($row["codigo"]==$globalUnidad)?"selected":""?> ><?=$row["abreviatura"];?></option>
									    	<?php 
												 		} 
										 	?>
												</select>			                			
                  	...
                  	<div class="col-sm-6">
                  		<div class="row">
			                 ...
			                 <div class="col-sm-8">
			                	<div class="form-group">
	                              <select class="selectpicker form-control form-control-sm" name="area_costo[]" id="area_costo" data-style="select-with-transition" multiple data-actions-box="true" required data-live-search="true">
			  	                     <?php
				                     ...
				                     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				                     	...
				                     	$abrevX=$row['abreviatura'];
				                     ?>
				                     <option value="<?=$codigoX;?>" <?= $codigoX == 12 ? 'selected' : ''; ?>><small><?=$abrevX;?></small></option>	
				                       <?php
			  	                       }
			  	                       ?>
			                       </select>
