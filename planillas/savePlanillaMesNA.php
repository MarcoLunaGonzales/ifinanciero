	...
	while ($rowUO=$stmtUO->fetch()){
		...
		while ($rowC = $stmtPersonal->fetch()) 
		{
			...
			if($cod_personalVerificacion==null){
				...
				$stmtInsertPlanillas->bindParam(':created_by',$created_by);
				$stmtInsertPlanillas->bindParam(':modified_by',$modified_by);
				$stmtInsertPlanillaDetalle->bindParam(':procesado_reprocesado',$procesado_reprocesado);
				$flagSuccessIP=$stmtInsertPlanillas->execute();
				...
				$sqlDelete="DELETE from planillas_personal_mes where cod_planilla=$cod_planilla and procesado_reprocesado=1";
				$stmtDeletePersonal = $dbhIPD->prepare($sqlDelete);			
