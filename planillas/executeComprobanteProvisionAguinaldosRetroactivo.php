...
if($sw_auxiliar==0){//sin  distribucion de sueldos pendientes
  ...
  $glosaCabecera="Personal IBNORCA provisión de aguinaldos y provisión de indemnizaciones correspondientes al retroactivo de: ".$namemesPlanilla." ".$anioPlanilla; 
  $anioActual=date("Y");
  $mesActual=date("m");
  $diaActual=date("d");    
    ...
    if($cod_area_contabilizacionX==11){//area de OI debe ser por oficina
      ...
      while ($rowXY = $stmtXY->fetch(PDO::FETCH_ASSOC)) {
        ...
        if($monto_area>0){
          ...
          $glosaDetalle1=$nombre_area." Duodecimas de aguinaldo correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;
          $glosaDetalle1=$nombre_area." Duodecimas de aguinaldo correspondiente: ".$namemesPlanilla."/".$anioPlanilla;
          ...
          if($bandera_segundoAguinaldo==1){//1 si 0 no
            $totalProvision+=$montoAguinaldo;
            $glosaDetalle1=$nombre_area." Duodecimas de segundo aguinaldo correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;
            $sqlInsertDet="INSERT INTO comprobantes_detalle (cod_comprobante, cod_cuenta, cod_cuentaauxiliar, cod_unidadorganizacional, cod_area, debe, haber, glosa, orden) VALUES ('$codComprobante','$cod_cuenta','0','$codigoUOXY','$cod_area_contabilizacionX','$montoAguinaldo','0','$glosaDetalle1','$ordenDetalle')";
            $stmtInsertDet = $dbh->prepare($sqlInsertDet);
            $flagSuccessDet=$stmtInsertDet->execute();
            $ordenDetalle++;
  ...
  //contra cuenta
  $glosaDetalleGeneral="Provisión de aguinaldo correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;      
  $glosaDetalleGeneral2="Provisión de segundo aguinaldo correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;      
    ...
    if($cod_area_contabilizacionX==11){//area de OI debe ser por oficina
      ...
      while ($rowXY = $stmtXY->fetch(PDO::FETCH_ASSOC)) {
        ...
        if($monto_area>0){
          ...
          $glosaDetalle1=$nombre_area." Provisión de indemnizaciones correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;
          $sqlInsertDet="INSERT INTO comprobantes_detalle (cod_comprobante, cod_cuenta, cod_cuentaauxiliar, cod_unidadorganizacional, cod_area, debe, haber, glosa, orden) VALUES ('$codComprobante','$cod_cuenta','0','$codigoUOXY','$cod_area_contabilizacionX','$montoAguinaldo','0','$glosaDetalle1','$ordenDetalle')";
          $stmtInsertDet = $dbh->prepare($sqlInsertDet);
          $flagSuccessDet=$stmtInsertDet->execute();
          $ordenDetalle++;
  ...
  //contra cuenta
  $glosaDetalleGeneral="Provisión de indemnizaciones correspondiente al retroactivo de: ".$namemesPlanilla."/".$anioPlanilla;      
  $codUOCentroCosto=$globalUnidadX;
  $codAreaCentroCosto="502";
  //SUELDOS POR PAGAR
  // $totalProvision=totalLiquidoPagable($gestionPlanilla, $mesPlanilla);
