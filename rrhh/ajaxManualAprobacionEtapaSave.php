...
try {
  if($metodo == 1){
    // Registro
    $sql  = "INSERT INTO manuales_aprobacion_etapas (cod_etapa,cod_cargo,nombre,descripcion,nro_etapa) VALUES (:cod_etapa,:cod_cargo,:nombre,:descripcion,:nro_etapa)";
    ...
    // Actualización
    $sql = "UPDATE manuales_aprobacion_etapas SET cod_etapa = :cod_etapa, cod_cargo = :cod_cargo, nombre = :nombre, descripcion = :descripcion, nro_etapa = :nro_etapa WHERE codigo = :codigo";
    $stmt = $dbh->prepare($sql);
    $stmt->bindParam(':cod_etapa', $cod_etapa);
    $stmt->bindParam(':cod_cargo', $cod_cargo);
    $stmt->bindParam(':nombre', $nombre);
