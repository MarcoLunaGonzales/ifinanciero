...
if ($codigo > 0){
    ...
    $codigo=$codigo;    
    $stmt = $dbh->prepare("SELECT * FROM tipos_afp where codigo =:codigo");
    //Ejecutamos;
...
<div class="content">
	<div class="container-fluid">
		...
		<div class="col-md-12">
		  <form id="form1" class="form-horizontal" action="<?=$urlSaveTipos_afp;?>" method="post">
  
			<div class="card">
			  <div class="card-header <?=$colorCard;?> card-header-text">
				<div class="card-text">
				  <h4 class="card-title"><?php if ($codigo == 0) echo "Registrar"; else echo "Editar";?>  <?=$nombreSingularTipos_afp;?></h4>
				</div>
			  </div>
			  <div class="card-body ">
    ...
    <div class="form-group">
        <input class="form-control" type="text" name="abreviatura" id="abreviatura" required="true" value="<?=$abreviatura;?>" onkeyup="javascript:this.value=this.value.toUpperCase();"/>
			  ...
			  <div class="card-footer ml-auto mr-auto">
				...
				<a href="<?=$urlListTipos_afp;?>" class="<?=$buttonCancel;?>">Cancelar</a>
			  </div>
			</div>
		  </form>
		</div>
