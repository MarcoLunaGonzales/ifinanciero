...
// AREA ORGANIZACIÓN
$codUnidad = $_SESSION["globalUnidad"];
$sqlAO = "SELECT ao.codigo
        FROM areas_organizacion ao
        WHERE ao.cod_unidad = '$codUnidad'
        AND ao.cod_area = '$codArea'";
// echo $sqlAO;
$stmtAO = $dbh->prepare($sqlAO);
$stmtAO->execute();
$resultado = $stmtAO->fetch(PDO::FETCH_ASSOC);
if ($resultado !== false && isset($resultado['codigo'])) {
    $cod_areaorganizacion = $resultado['codigo'];
} else {
    $cod_areaorganizacion = '';
}


// Preparamos
$sql="SELECT c.codigo, UPPER(c.nombre) as nombre,
    (SELECT count(*) FROM cargos_areasorganizacion ca WHERE c.codigo=ca.cod_cargo AND ca.cod_areaorganizacion='$cod_areaorganizacion') AS bandera,
                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <form id="lñp" method="post" action="?opcion=areasCargosSave">
                      <input type="hidden" name="cod_areaorganizacion" value="<?=$cod_areaorganizacion?>">
