...
if($cod_estadoreferencial==1){//insertar
	$sql="INSERT INTO cargos_funciones(cod_cargo,nombre_funcion,peso,cod_estado) values($cod_cargo,'$nombre_funcion',$peso,$cod_estadoreferencial) ";	
	$stmtU = $dbhU->prepare($sql);
	$flagsucces=$stmtU->execute();

	
}elseif($cod_estadoreferencial==2){//actualizar
	$sql="UPDATE cargos_funciones set nombre_funcion='$nombre_funcion',peso=$peso where cod_funcion=$cod_funcion";
	$stmtU = $dbhU->prepare($sql);
	$flagsucces=$stmtU->execute();
