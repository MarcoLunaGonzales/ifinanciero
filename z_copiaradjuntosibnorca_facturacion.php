...
require_once 'conexion.php';
    ...
    $sql="SELECT codigo,direccion_archivo as origen,cod_solicitud_facturacion,descripcion  from archivos_adjuntos_solicitud_facturacion where (cod_archivoibnorca=0 or cod_archivoibnorca is null) and cod_solicitud_facturacion in (5027,5019);";		
    ...
    while ($rowDetalle = $stmt->fetch(PDO::FETCH_ASSOC)) {
       ...
       $codigo=$rowDetalle['cod_solicitud_facturacion'];
       $array1=explode("/",$dir);
       ...
       if (file_exists($dirCorrecto)) {
          ...
          if (is_readable($dirCorrecto)){
            ...
            if($flagSuccess){
              $sqlDocumentoAlter="UPDATE archivos_adjuntos_solicitud_facturacion set cod_archivoibnorca=$codigoFila where codigo=$codigoArchivo";    
