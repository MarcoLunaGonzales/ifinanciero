...
$sql="SELECT p.codigo, p.nombre, p.nro_cuenta, p.direccion, p.telefono, p.referencia1, p.referencia2, 
  (select b.nombre from bancos b where b.codigo=p.cod_banco)cod_banco FROM cuentas_auxiliares_cajachica p where cod_estadoreferencial=1 and p.cod_cuenta='$codigoCuentaPadre' order by p.nombre";
//echo $sql;
...
?>
                        <tr>
                          ...
                          <td class="td-actions text-right">
                            ...
                            if($globalAdmin==1){
                            ?>
                            <a href='<?=$urlEditCC;?>&codigo=<?=$codigo;?>&codigo_padre=<?=$codigoCuentaPadre?>' rel="tooltip" class="<?=$buttonEdit;?>">
                              <i class="material-icons"><?=$iconEdit;?></i>
                            </a>
                            <button rel="tooltip" class="<?=$buttonDelete;?>" onclick="alerts.showSwal('warning-message-and-confirmation','<?=$urlDeleteCC;?>&codigo=<?=$codigo;?>&codigo_padre=<?=$codigoCuentaPadre?>')">
...
?>
              ...
              <div class="card-footer fixed-bottom">
                    <button class="<?=$buttonNormal;?>" onClick="location.href='<?=$urlRegisterCC;?>&codigo=<?=$codigoCuentaPadre;?>'">Registrar</button>
              </div>		  
            </div>
          </div>  
        </div>
