...
while ($row = $lista->fetch(PDO::FETCH_ASSOC)) {
  ...
  <tr class="fila_proveedor<?=$fila?>">
    ...
    <td class="text-right">
      ...
      if(($importe-$pagado)>0){
        ?>
        <input type="number" step="any" required min="1000" class="form-control text-right text-success" value="0" id="monto_pago<?=$index?>PPPP<?=$fila?>" name="monto_pago<?=$index?>PPPP<?=$fila?>"> <?php
      }else{ ?>
        <input type="number" step="any" required min="1000" class="form-control text-right text-success" readonly value="0" id="monto_pago<?=$index?>PPPP<?=$fila?>" name="monto_pago<?=$index?>PPPP<?=$fila?>"> <?php
      } ?>
    </td>
