...
try {
    ...
    if ($_POST["codigo"] == 0){
        ...
        $stmt2 = $dbh->prepare("INSERT INTO activofijos_asignaciones(cod_activosfijos,fechaasignacion,
            cod_ubicaciones,cod_personal, estadobien_asig)
        ...
        //-------------------------------------------------------------
        //insertar la imagen $image = $_FILES['image']['name'];
        ...
        vidautilmeses=:vidautilmeses,estadobien=:estadobien,otrodato=:otrodato,
        cod_ubicaciones=:cod_ubicaciones,cod_empresa=:cod_empresa,activo=:activo,
        cod_responsables_responsable=:cod_responsables_responsable,cod_responsables_autorizadopor=:cod_responsables_autorizadopor,
         vidautilmeses_restante=:vidautilmeses_restante,cod_af_proveedores=:cod_af_proveedores,
         numerofactura=:numerofactura, bandera_depreciar = :bandera_depreciar where codigo = :codigo");
        //bind
        ...
        //si es un personal diferente asignado, hay q insertar en asignaciones...
        if($idubicacion222 != $cod_ubicaciones OR $idresponsable222 !=$cod_responsables_responsable){
            $stmt2 = $dbh->prepare("INSERT INTO activofijos_asignaciones(cod_activosfijos,fechaasignacion,
            cod_ubicaciones,cod_personal, estadobien_asig)
            values (:cod_activosfijos, now(),
            :cod_ubicaciones, :cod_personal, :estadobien_asig)");
            $stmt2->bindParam(':cod_activosfijos', $codigo);
            //$stmt2->bindParam(':fechaasignacion', $fechalta);
            $stmt2->bindParam(':cod_ubicaciones', $cod_ubicaciones);
            $stmt2->bindParam(':cod_personal', $cod_responsables_responsable);
            $stmt2->bindParam(':estadobien_asig', $estadobien);
